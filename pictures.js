...
function getPicture(word) {
	...
	req.onload = function () {
		if (req.status >= 200 && req.status < 400) {
			...
			entry.id = 'card-svg-' + word
			document.getElementById('pictures').insertBefore(entry, document.getElementById('pictures').childNodes[0])
			
			Potrace.loadImageFromUrl(url)
			Potrace.process(function(){
				var svgdiv = document.getElementById('card-svg-' + word)
				svgdiv.style.display = 'inline-block'
				svgdiv.innerHTML = Potrace.getSVG(0.5)
			})
		}
	req.onerror = function() {
		console.log('Sorry, there was an error getting pictures from Flickr.')
	}
	}
	req.send()
}
